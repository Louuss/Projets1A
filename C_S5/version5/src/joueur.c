...
//        if (i < j->n_ami){
//            int numero_ami = get_numero_maitre(j->tab_ami[i]);
//            ami_mort_plateau(numero_ami);
...
void afficher_main(joueur j) {
    ...
    while (i < NB_CARTE_BEGIN) {
        ...
        } else if (strcmp(get_nom(j->main[i]), "hell") == 0 || strcmp(get_nom(j->main[i]), "slow") == 0) {
            printf("|"ANSI_COLOR_RED ANSI_BOLD"   %s   "ANSI_COLOR_RESET ANSI_BOLD_OFF"|  ", get_nom(j->main[i]));
        } else {
            printf("|"ANSI_COLOR_RED ANSI_BOLD"  %s"ANSI_COLOR_RESET ANSI_BOLD_OFF"   |  ", get_nom(j->main[i]));
        }
...
int get_priorite(joueur j){
    //TODO
    
    return j->temps;
}

int get_cout_action(joueur j){
    //TODO
    
    return j->cout_action;
}
...
void set_priorite(joueur j, int i){
    //TODO
    
    j->temps = i;   
}

void set_cout_action(joueur j, int i){
    //TODO
