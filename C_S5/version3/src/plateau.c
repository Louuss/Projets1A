...
void faire_jouer_ami(joueur j, joueur adversaire) {
    if (get_nb_ami(j) != 0) {
        for (int z = 0; z < get_nb_ami(j); z++) {
            ...
            if (adversaire_vivant(get_numero_cible(a)) != NULL) {
                ad = adversaire_vivant(get_numero_cible(a));
            } else {
                ad = adversaire;
                set_numero_cible(a,get_numero(adversaire));
            }
